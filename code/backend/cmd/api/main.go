...
func run() error {
	...
	db, err := sql.Open("pgx", databaseURL); if err != nil { return fmt.Errorf("open database: %w", err) }
	defer db.Close()
...
func decodePatch(w http.ResponseWriter, r *http.Request) (patchTaskRequest, []fieldError, bool) {
	...
	for key, body := range raw {
		...
		case "description":
			...
			if string(body) != "null" { var v string; if err := json.Unmarshal(body, &v); err != nil { return req, nil, true }; v = strings.TrimSpace(v); if v != "" { req.Description.value = &v } }
		case "status":
			var v string; if err := json.Unmarshal(body, &v); err != nil { return req, nil, true }; req.Status = &v
		case "due_date":
			req.DueDate.set = true
			if string(body) != "null" { var v string; if err := json.Unmarshal(body, &v); err != nil { return req, nil, true }; v = strings.TrimSpace(v); req.DueDate.value = &v }
		}
	}
...
func scanTask(row taskScanner) (task, error) {
	var item task; var description, dueDate sql.NullString; var createdAt, updatedAt time.Time
	...
	item.CreatedAt = createdAt.UTC().Format(time.RFC3339); item.UpdatedAt = updatedAt.UTC().Format(time.RFC3339)
	return item, nil
}
...
func writeError(w http.ResponseWriter, r *http.Request, status int, code, message string, details []fieldError) {
	requestID := r.Header.Get("X-Request-Id"); if requestID == "" { requestID = fmt.Sprintf("%d", time.Now().UnixNano()) }
	w.Header().Set("X-Request-Id", requestID); w.Header().Set("Content-Type", "application/json; charset=utf-8")
	var body errorResponse; body.Error.Code = code; body.Error.Message = message; body.Error.Details = details; if body.Error.Details == nil { body.Error.Details = []fieldError{} }; body.Error.RequestID = requestID
	w.WriteHeader(status); _ = json.NewEncoder(w).Encode(body)
}
