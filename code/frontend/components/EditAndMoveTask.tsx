...
function validate(task: Task): Errors {
  ...
  if (task.title.trim().length > 120) errors.title = "Title must be 120 characters or fewer.";
  if ((task.description ?? "").trim().length > 2000) errors.description = "Description must be 2,000 characters or fewer.";
  if (!validDate(task.due_date ?? "")) errors.due_date = "Due date must be a valid calendar date.";
    ...
    <section className={styles.shell} aria-label="Edit and move tasks">
      ...
      <div className={styles.board} aria-label="Task board">
        {statuses.map((status) => {
          ...
          return <section className={styles.column} key={status.key}><ColumnHeader status={status} count={columnTasks.length} />
            <div className={styles.cards}>{columnTasks.length ? columnTasks.map((task) => <article className={styles.card} key={task.id}>
              <h3>{task.title}</h3><p>{task.description || "No description"}</p>{task.due_date && <span className={styles.meta}>Due {new Date(`${task.due_date}T00:00:00`).toLocaleDateString("en", { month: "short", day: "numeric", year: "numeric" })}</span>}
              <span className={`${styles.pill} ${styles[task.status]}`}>{statuses.find((item) => item.key === task.status)?.label}</span>
              <div className={styles.actions}><button disabled={task.status === "todo"} onClick={() => moveTask(task, -1)}>Move left</button><button disabled={task.status === "done"} onClick={() => moveTask(task, 1)}>Move right</button><button ref={opener} onClick={() => setEditing(task)}>Edit</button></div>
            </article>) : <div className={styles.empty}><span aria-hidden="true">□</span>No {status.label.toLowerCase()} tasks.</div>}</div>
          </section>;
        })}
      </div>
      {editing && <EditModal task={editing} errors={errors} onCancel={() => { setEditing(null); opener.current?.focus(); }} onSave={saveTask} />}
    </section>
  );
}
...
function EditModal({ task, errors, onCancel, onSave }: { task: Task; errors: Errors; onCancel: () => void; onSave: (task: Task) => void }) {
  ...
  return <div className={styles.backdrop} onMouseDown={onCancel}><form className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="edit-title" onSubmit={submit} onMouseDown={(event) => event.stopPropagation()}>
    <header><h2 id="edit-title">Edit task</h2><button type="button" onClick={onCancel}>Close</button></header>
    <Field id="edit-task-title" label="Title" error={errors.title}><input autoFocus id="edit-task-title" value={draft.title} aria-describedby={errors.title ? "edit-task-title-error" : undefined} onChange={(e) => setDraft({ ...draft, title: e.target.value })} /></Field>
    <Field id="edit-task-description" label="Description" error={errors.description}><textarea id="edit-task-description" value={draft.description ?? ""} onChange={(e) => setDraft({ ...draft, description: e.target.value })} /></Field>
    <Field id="edit-task-due" label="Due date" error={errors.due_date}><input id="edit-task-due" type="date" value={draft.due_date ?? ""} onChange={(e) => setDraft({ ...draft, due_date: e.target.value || null })} /></Field>
    <Field id="edit-task-status" label="Status" error={errors.status}><select id="edit-task-status" value={draft.status} onChange={(e) => setDraft({ ...draft, status: e.target.value as TaskStatus })}>{statuses.map((s) => <option value={s.key} key={s.key}>{s.label}</option>)}</select></Field>
